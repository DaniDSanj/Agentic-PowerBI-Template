...
1. **Crea el repo del cliente a partir de esta plantilla** (no hagas `git clone` de este repositorio — eso arrastraría su historial de git; usa el mecanismo de "template" para partir de un repo nuevo y limpio):
   ...
   ```
   o el botón **"Use this template"** en GitHub. El repo resultante nace con `.claude/`, `.github/workflows/validate-pr.yml`, `tools/ci/`, `files/context/` y `CLAUDE.md` ya en la raíz — nada que copiar a mano. **No marques "Include all branches"** (ni pases `--include-all-branches`): "Use this template" solo copia la rama por defecto (`main`) salvo que se marque esa opción, y eso es lo que queremos — `dev` no debe venir copiada, la crea el script del punto siguiente.
2. **Ejecuta `tools/setup-branch-protection.ps1` una vez**, justo después de clonar el repo cliente (requiere `gh` autenticado con permisos de administrador del repo). Crea la rama `dev` desde `main` y protege ambas — sin este paso el repo cliente queda sin ninguna protección de rama, aunque el fichero del workflow ya esté ahí. Si el repo cliente es privado con plan Free, la API de branch protection devuelve un 403 (`Upgrade to GitHub Pro or make this repository public`) — decide con el cliente si el repo pasa a público o si asume GitHub Pro antes de continuar (ver `files/context/limites-duros.md`, nunca se introduce una herramienta de pago sin su autorización explícita).
4. Abre el repo del cliente (ya clonado en tu máquina) con Claude Code **desde esa carpeta** — los hooks de `.claude/settings.json` están arraigados al directorio de proyecto de la sesión (`CLAUDE_PROJECT_DIR`), así que solo se disparan si Claude Code se abre con esa carpeta como raíz de sesión, no editando sus ficheros desde otra ubicación.
5. En la primera sesión, si `.claude/project-config.json` no existe (siempre será así en un repo recién creado desde la plantilla), el hook `session-start-check` y `files/context/flujo-trabajo.md` te dirigirán a invocar el skill `requirements-intake` — no empieces por ningún otro sitio. Ese mismo paso 0 de `requirements-intake` es también donde se fija `TABULAR_EDITOR_PATH` y el resto de valores específicos de esta máquina/cliente (ver punto 6) — no antes.
6. Instala en la máquina la toolchain que asumen los hooks: **Power BI Desktop, Tabular Editor 2 (>= 2.20.0, sep-2023 — versiones anteriores no reconocen los `.tmdl` que genera Desktop actual; instala la última con `winget install TabularEditor.TabularEditor.2`), DAX Studio**, y **Python** si el proyecto va a operar en Escenario B (semantic-link-labs). Instala también **`gitleaks`** (https://github.com/gitleaks/gitleaks) si quieres que el hook local `pre-commit-secrets-check.ps1` bloquee de verdad un commit con un secreto — sin él, el hook solo avisa y deja pasar (la capa autoritativa sigue siendo el job `gitleaks` de CI). Si `TabularEditor.exe` no está en el `PATH`, define `TABULAR_EDITOR_PATH` en el campo `"env"` de nivel raíz de `.claude/settings.json` — **no** con `setx`/variable de entorno de sistema, verificado que una sesión de Claude Code no la hereda de forma fiable ni siquiera arrancando después de fijarla:
   ```json
...
**Otra contrapartida del mismo mecanismo, distinta de la anterior**: "Use this template" copia ficheros, pero de las ramas **solo copia la rama por defecto** (`main`, nunca `dev` — no marques "Include all branches"), y **no copia ninguna configuración del repo** — branch protection, `security_and_analysis`, webhooks, secrets de Actions. Cada repo cliente nace con **solo `main`, sin protección alguna** hasta que se ejecuta `tools/setup-branch-protection.ps1` (paso 2 más arriba), que crea `dev` desde `main` y protege ambas. No darlo por hecho solo porque la plantilla-origen lo tenga configurado — verificarlo explícitamente (`gh api repos/<owner>/<repo>/branches/main/protection`) en cada repo cliente nuevo.
...
- **Rama `dev` + branch protection + escaneo de secretos (esta sesión) — aplicado y verificado en esta plantilla, con un hallazgo real que cambió el plan a mitad de sesión.** Se creó `dev`, se ampliaron `.github/workflows/validate-pr.yml` (jobs `gitleaks` y `source-branch-gate`) y `.claude/settings.json` (hook `PreToolUse` + deny de push a `dev`). Al ejecutar `tools/setup-branch-protection.ps1` contra el repo (entonces privado), la API de GitHub devolvió `403 Upgrade to GitHub Pro or make this repository public` — **branch protection completa (clásica y rulesets) no está disponible en absoluto para un repo privado en plan Free**, no solo el secret scanning nativo como se asumía al planear. El script además tenía un bug real: no comprobaba el exit code de `gh api` y reportaba "OK" aunque la llamada hubiera fallado con 403 — corregido para que falle de forma explícita. Ante el bloqueo, el usuario decidió hacer público el repo en vez de pagar GitHub Pro (plantilla sin datos de cliente ni credenciales). Tras el cambio de visibilidad: `tools/setup-branch-protection.ps1` corrió con éxito (con un segundo bug real corregido en el payload — la API rechaza `contexts` y `checks` combinados, solo acepta `checks`), y se confirmó con `gh api .../branches/{main,dev}/protection` (GET, no solo el exit code) que la protección quedó realmente aplicada; se habilitó también el secret scanning + push protection nativos de GitHub (gratis ya en público) como capa adicional a `gitleaks`. **Segundo hallazgo real, detectado por el usuario y corregido en la misma sesión**: el propio script, al terminar, cambiaba el `default_branch` del repo a `dev` — eso rompía "Use this template", que solo copia el `default_branch` salvo que se marque "Include all branches"; un repo cliente nuevo habría nacido con una sola rama y sin `main`. Corregido: el script ya no toca `default_branch`, que se mantiene siempre en `main`. Pendiente de verificar todavía: una PR real de prueba contra `main` desde una rama que no sea `dev` (para confirmar que `source-branch-gate` la bloquea de verdad en la práctica), un hallazgo real de `gitleaks` en CI, y la creación de un repo cliente real con "Use this template" para confirmar que efectivamente nace con solo `main` y que `setup-branch-protection.ps1` funciona ahí igual que en la plantilla.
