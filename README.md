...
**Otra contrapartida del mismo mecanismo, distinta de la anterior**: "Use this template" copia ficheros y ramas, pero **no copia configuración del repo** — branch protection, webhooks, secrets de Actions. Cada repo cliente nace con `dev`/`main` sin ninguna protección hasta que se ejecuta `tools/setup-branch-protection.ps1` (una vez, justo tras el primer push). No darlo por hecho solo porque la plantilla-origen lo tenga configurado — verificarlo explícitamente (`gh api repos/<owner>/<repo>/branches/main/protection`) en cada repo cliente nuevo.
