...
2. Copia `project-config.example.json` (de este mismo `.claude/`) a `.claude/project-config.json` y rellena `proyecto`, `escenario`, `storageMode` (una vez conocido, ver Paso 1) y `fechaBootstrap`.
3. Este fichero es lo que leen el resto de skills/hooks para bifurcar su comportamiento — no lo dejes con placeholders (`A|B`, cadenas vacías).
4. **Patchea `.claude/settings.json` con los valores específicos de esta máquina/cliente** — nunca asumas que los valores heredados de la plantilla sirven tal cual, y nunca dejes en el repo del cliente una ruta absoluta de una máquina distinta:
