...
**Pendiente de dogfooding real**, igual que el resto de la capa de gobernanza local descrita en la entrada siguiente: falta confirmar en una sesión real que `-LocalGuardOnly` funciona sin `gh` instalado, y que el automatismo al final de los modos que tocan GitHub activa el guard local correctamente sobre un repo cliente privado real.
...
**Pendiente de dogfooding real** (a diferencia de la mayoría de entradas de este CHANGELOG): este diseño no se ha probado todavía contra un repo cliente privado real — falta confirmar en una sesión real que `core.hooksPath` se activa correctamente en Windows vía el shim `#!/bin/sh`, que git realmente invoca `tools/git-hooks/pre-push` con el stdin esperado, y que el fail-closed de `gitleaks` en repos privados no produce falsos bloqueos. No dar por bueno sin repetir esa validación (mismo criterio que el resto de la plantilla).
